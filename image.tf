...
# =============================================================================
locals {
  user_acct_id = "${substr(element(split("a/", ibm_is_vpc.f5_vpc01.resource_crn), 1),0,32)}"
}
...
resource "ibm_iam_authorization_policy" "authorize_image" {
  #depends_on                    = ["ibm_is_vpc.f5_vpc01"]
  provider                      = "ibm.vfnsvc"
...
data "external" "create_image_hack" {
  ...
  program = ["bash", "${path.module}/create-image-hack.sh"]
...
#   #   command = "${path.module}/create-image-hack.sh"
#   #   environment = {
#   #     ibmcloud_api_key = "${var.ibmcloud_api_key}"
#   #     generation = "${var.generation}"
#   #     region = "${var.region}"
