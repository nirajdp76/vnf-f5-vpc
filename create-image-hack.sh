...
function parse_input() {
    if [[ -z "${ibmcloud_api_key}" ]] || [[ -z "${generation}" ]] || [[ -z "${region}" ]] || [[ -z "${resource_group_id}" ]] || [[ -z "${f5_image_name}" ]] || [[ -z "${vnf_f5bigip_cos_image_url}" ]]; then
        eval "$(jq -r '@sh "ibmcloud_api_key=\(.ibmcloud_api_key) generation=\(.generation) region=\(.region) resource_group_id=\(.resource_group_id) f5_image_name=\(.f5_image_name) vnf_f5bigip_cos_image_url=\(.vnf_f5bigip_cos_image_url)"')"
    fi
...
function prep_deps() {
    ...
    ibmcloud login --apikey "${ibmcloud_api_key}" -r "${region}" -g "${resource_group}" &> $MSG_FILE
